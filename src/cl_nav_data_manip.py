...
def dummy_and_concat(df, col, prefix):
    if isinstance(col, list) and isinstance(prefix, list):
        ...
        for idx, column in enumerate(col):
            dummy_df = pd.get_dummies(df_new[col[idx]], prefix=prefix[idx], prefix_sep='_', drop_first=True)
            df_new = pd.concat([df_new, dummy_df], axis=1)
        df_new.drop(col, axis=1, inplace=True)
        return df_new
    else:
        dummy_df = pd.get_dummies(df[col], prefix=prefix, prefix_sep='_', drop_first=True) #prefix=prefix
        df = pd.concat([df, dummy_df], axis=1)
        df.drop(col, axis=1, inplace=True)
        return df

def readable_cols(df):
...
if __name__ == '__main__':
    ...
    dummy_ls = ['Service_Code', 'Procedure_Description', 'Financial_Class', 'NCI_Transaction_Detail', 'Admit_Type', \
                'Current_Patient_Class_Description', 'Discharge_Department_ID', 'Primary_Service_Name']

    df_dummies = dummy_and_concat(df, dummy_ls, dummy_ls)

    # drop all dates here
    df_dummies.drop(['Service_Date', 'Post_Date', 'Account_Discharge_Date', 'Account_Billed_Date',
    'Admit_Date', 'First_Claim_Date', 'Last_Claim_Date', 'Last_Payment_Date'], axis=1, inplace=True)

    # drop specific features here #'Unnamed: 0'
    df_dummies.drop(['account_id', 'Transaction_Number', 'TransactionType_Tdata',
    ...
    df_dummies['AWO_Bucket'] = df_dummies['AWO_Bucket'].map({'1. <0': 0, '2. 0-1,000': 1,
    '3. 1,000-2,500':2,  '4. 2,500-5,000':3, '5. 5,000-10,000':4, '6. 10,000+':5})

    #rename awo transation amount and drop original
    df_dummies['awo_amount'] = df_dummies['account_id.1']
    df_dummies.drop('account_id.1', axis=1, inplace=True)

    #lower case
    final_df = readable_cols(df_dummies)

    final_df.to_csv('../../navigant_data/final_df_cl_edit.csv')
